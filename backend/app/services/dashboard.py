...
class DashboardService:
    ...
    def get_dashboard(self, tenant_id: UUID) -> DashboardResponse:
        projects = self._project_summary(tenant_id)
        workers = self._worker_summary(tenant_id)
        reports = self._report_summary(tenant_id)
        materials = self._material_summary(tenant_id)
        clients = self._client_count(tenant_id)
        kpis = self._calculate_kpis(tenant_id, projects)
    ...
    def _project_summary(self, tenant_id: UUID) -> ProjectSummary:
    ...
    def _worker_summary(self, tenant_id: UUID) -> WorkerSummary:
        total = self.db.execute(
    ...
    def _report_summary(self, tenant_id: UUID) -> ReportSummary:
        today = date.today()
    ...
    def _material_summary(self, tenant_id: UUID) -> MaterialSummary:
        low = self.db.execute(
    ...
    def _client_count(self, tenant_id: UUID) -> int:
        return (
            self.db.execute(
    ...
    def _calculate_kpis(
        self, tenant_id: UUID, projects: ProjectSummary
    ) -> KpiData:
        ...
        active_workers = self.db.execute(
            select(func.count()).select_from(Worker).where(
                Worker.tenant_id == tenant_id,
                Worker.is_active.is_(True),
            )
